...
на основе анализа всех 11 продуктов портфолио. Загружает в S3, возвращает URL.
"""
import os
import io
import base64
...
def build_pdf_bytes():
    ...
    story.append(Paragraph(
        "Анализ 11 продуктов портфолио  \u00b7  4 направления услуг  \u00b7  Дорожная карта 2026-2029",
        styles["Cover_Sub"]
    ))
    ...
    summary = Table(
        [[Paragraph(
            ...
            "из 11 запущенных продуктов в 8 отраслях (агро, недвижимость, маркетинг, авиация, "
            "промышленность, бухгалтерия, EdTech). Основная проблема — все продукты разовые, нет SaaS-выручки. "
            "Стратегия на 3 года: <b>выделить 4 SaaS-флагмана</b> (MAIL-KA, RoomScan AI, MAT-AD, УЧИСЬПРО), "
            ...
            "<b>17 млн руб/мес</b>, из них MRR от продуктов — <b>47%</b>.",
            styles["Body"]
        )]],
        colWidths=[CW]
    )
    ...
    # ═══════════════════════════════════════════════════════════════════
    story += section_header("ЧАСТЬ I. АНАЛИЗ 11 ПРОДУКТОВ ПОРТФОЛИО", styles)
    ...
    products = [
        ...
        (11, "УЧИСЬПРО",       "EdTech / ИИ-репетитор",
         "ИИ-репетитор с голосовым ассистентом: 5-минутная диагностика пробелов, "
         "персональный маршрут подготовки к ЕГЭ/ОГЭ, цель — 90+ баллов. Работает 24/7 без записи.",
         "Школьники 9-11 классов, родители, репетиторы и онлайн-школы (B2B).",
         "B2C-подписка 990-2 990 руб/мес (ученик) + B2B-лицензии 50 000-300 000 руб/год школам.",
         "SaaS-ФЛАГМАН №4. Огромный рынок EdTech 90 млрд руб, AI+голос = уникальное УТП. "
         "Сезонность сентябрь-май, виральный B2C-вход."),
    ]
    ...
    story.append(colored_table([
        ...
        ["F. EdTech\n(новое направление)",
         "УЧИСЬПРО",
         "B2C-подписка + B2B-лицензии школам. MRR 2 млн руб/мес к Году 3."],
    ...
    swot = Table([
        ...
        [Paragraph(
            "• 11 запущенных продуктов в 8 отраслях\n"
            ...
            styles["Body"]),
         Paragraph(
            "• Распылённость: 11 продуктов = 11 фокусов\n"
    ...
    for t in [
        "Customer development: интервью клиентов всех 11 продуктов, сбор «было больше стало» в цифрах",
        "Переупаковать 11 кейсов с конкретными результатами (% роста, руб экономии)",
        "Сегментировать продукты по кластерам A-F (см. Часть II)",
    ...
    story.append(kpi_block(
        ...
        "Команда 65+  |  Telegram 1 500 подписчиков  |  11 кейсов с цифрами",
    ...
    story.append(kpi_block(
        "KPI ГОДА 3 — ФИНАЛ",
        "Выручка 17 000 000 руб/мес  |  MRR от SaaS: 8 000 000 руб (47%)  |  "
        "8 000+ платящих клиентов  |  Команда 140+  |  NPS 70+  |  Экспорт в 2 страны СНГ",
    ...
    story.append(colored_table([
        ...
        ["SaaS УЧИСЬПРО (B2C + B2B школы)",   "100 000",       "700 000",       "2 000 000"],
        ...
        ["ИТОГО ВЫРУЧКА",                       "4 600 000",     "9 700 000",     "17 000 000"],
        ["Доля MRR от SaaS",                   "11%",           "40%",           "47%"],
    ...
    story.append(colored_table([
        ...
        ["Y1 Q1",   "Аудит",        "Сегментация портфолио · Head of Product · 11 кейсов"],
        ["Y1 Q2",   "MAIL-KA SaaS", "Биллинг + закрытая beta + Telegram-канал"],
        ["Y1 Q3",   "Sales+АгроПорт","Sales Manager · 1-й корп. пилот · грант РФРИТ"],
        ["Y1 Q4",   "RoomScan + УЧИСЬПРО", "Pro-тариф недвижке · запуск B2C-биллинга УЧИСЬПРО к учебному году"],
        ["Y2 Q1",   "MAT-AD",       "Запуск 3-го SaaS · whitelabel MAIL-KA"],
        ["Y2 Q2",   "Корпоратив + EdTech", "СмартМаш — 2 завода · 5+ агрохолдингов · 10 школ на УЧИСЬПРО"],
        ["Y2 Q3",   "Бренд",        "Подкаст · РИФ · ProductSense · отчёт об AI"],
        ["Y2 Q4",   "Процессы",     "CSM-команда · собственная CRM · NPS 60+"],
        ["Y3 Q1",   "Экспорт",      "Локализация MAIL-KA и УЧИСЬПРО для Казахстана"],
        ["Y3 Q2",   "Лидерство",    "MAIL-KA №1-2 в РФ · УЧИСЬПРО в топ-3 EdTech · Open-source SDK"],
        ["Y3 Q3",   "Капитал",      "Подготовка к Series A или дивиденды"],
        ["Y3 Q4",   "Финал",        "17 млн руб/мес · 8000+ платящих · 140+ команда"],
    ...
    conclusion = Table(
        [[Paragraph(
            "<b>MAT Labs — это компания с уникальным портфолио из 11 запущенных продуктов</b>, "
            "которая сегодня недополучает выручку из-за распыления. "
            "<br/><br/>Стратегия 3 лет — <b>фокус на 4 SaaS-флагманах (MAIL-KA, RoomScan AI, MAT-AD, УЧИСЬПРО)</b>, "
            "<b>построение корпоративной вертикали (АгроПорт + СмартМаш)</b>, "
            "<b>выход в EdTech через УЧИСЬПРО</b> и "
            "<b>сохранение услуг как канала продаж в продукты</b>. "
            "<br/><br/>Реалистичная цель — <b>выручка 17 млн руб/мес к концу 3 года</b>, "
            "из которых <b>47% — повторяющиеся (MRR)</b>. "
